function startEffect3() {
    window.interval[2] = setInterval(runEffect3, window.SPEED_EFFECT);
}
    ...
    if (typeof shuffle !== 'undefined') {
        ...
        $('.grid-selector > .grid-item').each(function () {
            ...
            if ($.inArray(unique, shuffle) !== -1) {
                ...
                var RANDOM_ANGLE_EFFECT = Math.ceil(Math.random() * 1);
                ...
                switch (RANDOM_ANGLE_EFFECT) {
                    ...
                    default:
                    case 1:
                        $selector.addClass('magictime vanishOut');
...
function endEffect3() {
    if (typeof window.interval !== 'undefined' && typeof window.interval[2] !== 'undefined') {
        clearInterval(window.interval[2]);
    }
}
